package com.alessandrosgarabottolo.session4.usefulmatrices;
...
public class UsefulMethodsMatricesVectors {
	...
	public static int[] buildHistogram(double[] realizations, double minBin, double maxBin, int binsNumber) {
		...
		double binSize = (maxBin - minBin) / binsNumber; // the bins have uniform length
		for (double realization : realizations) {
			if (realization < minBin) { // it goes in the first bin
				...
				/*
				 * bin[i+1] hosts the realization such that (realization - minBin) / binSize is in [i,i+1). 
				 * Remember: in Java the first entry is 0, and here bin[0] hosts the
				 * realization smaller than min
				 */
				bins[intRatio + 1] += 1;
			}
		}
